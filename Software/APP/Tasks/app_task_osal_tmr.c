...
//    static BOOL     chang_flag = FALSE;
    ...
    */
    if( task_event & OS_EVT_TMR_SEC ) {        
            sCtrl.Dtu.ConnectTimeOut++;
            sCtrl.Mtr.ConnectTimeOut++;
            sCtrl.Otr.ConnectTimeOut++;
            
            if( sCtrl.Dtu.ConnectTimeOut > 1)
                sCtrl.Dtu.ConnectFlag = 0;
            
            if( sCtrl.Mtr.ConnectTimeOut > 1)
                sCtrl.Mtr.ConnectFlag = 0;
