...
void App_ModbusInit(void)
{
    ...
    */
    ...
    sCtrl.Mtr.pch       = pch;                      // ... modbus控制块和全局结构体建立连接
    
#endif
    // UART2
    /***********************************************
    * 描述:   UART2串口和无线发送模块连接。master
    *         用作主机 通过无线模块  连接 公司无线接收服务器
    */
    ...
    sCtrl.Dtu.pch       = pch;                      // ... modbus控制块和全局结构体建立连接
#endif
...
//           ||   SLAVE_ADDR_DIP2 == SourceAddr    
        ...
        {
//            sCtrl.Mtr.RxCtrl.SourceAddr = SourceAddr;           //源地址
//            sCtrl.Mtr.RxCtrl.DestAddr   = GetRecSlaveAddr();    //目标地址
//            sCtrl.Mtr.RxCtrl.FramNum    = GetRecvFrameNbr();    //接收到的帧号
//            sCtrl.Mtr.RxCtrl.Code       = 0;                    //指令码 ，测量装置未定义 
//            sCtrl.Mtr.RxCtrl.Len        = Len;                  //数据区长度
            ...
            //取数据记录，将数据记录保存到接收区
            //OS_CRITICAL_ENTER();
//            memcpy( (INT08U *)&sCtrl.Mtr.Rd.Buf[0], (INT08U *)&pch->RxFrameData[DataPos], Len );
            //memcpy( (INT08U *)&sCtrl.Mtr.Rd.Buf[0], buf, Len );
            memcpy( (INT08U *)&sCtrl.Mtr.Rd.Buf[0], buf, sCsncPara.datalen );
            //OS_CRITICAL_EXIT();
            ...
            */       
            ...
            {
//                if( sCtrl.Mtr.ConnCtrl[i].SlaveAddr == SourceAddr)
                if( sCtrl.Mtr.ConnCtrl[i].SlaveAddr == sCsncPara.sourceaddr )
                {
                    sCtrl.Mtr.ConnCtrl[i].RecvEndFlg   = 1;      //接收到数据，置1。数据处理后置0
                    sCtrl.Mtr.ConnCtrl[i].TimeOut      = 0;      //超时计数器清零。
                }
            }
        ...
        sCtrl.Mtr.ConnectFlag       = 1;
        sCtrl.Mtr.ConnectTimeOut    = 0;   
...
//           ||      SLAVE_ADDR_SET  == SourceAddr    
        ...
        {
//            sCtrl.Dtu.RxCtrl.SourceAddr = SourceAddr;           //源地址
//            sCtrl.Dtu.RxCtrl.DestAddr   = GetRecSlaveAddr();    //目标地址
//            sCtrl.Dtu.RxCtrl.FramNum    = GetRecvFrameNbr();    //接收到的帧号
            ...
            */                   
            ...
            {
                memcpy( (INT08U *)&sCtrl.Dtu.Rd.Buf[0],   (INT08U *)&buf[0], sCsncPara.datalen );
            }
            /***********************************************
            * 描述： UART3连接状态描述
            */       
        ...
        sCtrl.Dtu.ConnectFlag       = 1;
        sCtrl.Dtu.ConnectTimeOut    = 0;   
...
//            sCtrl.Otr.RxCtrl.FramNum    = GetRecvFrameNbr();    //接收到的帧号
            ...
            */                   
            ...
            {
                memcpy( (INT08U *)&sCtrl.Otr.Rd.Buf[0],   (INT08U *)&buf[0], sCsncPara.datalen );
            }
            /***********************************************
            * 描述： UART2连接状态描述
            */       
            for(uint8 i = 0;i< COMM_DEV_OTR_CONN_NUM;i++)
            {
                ...
                if( sCtrl.Otr.ConnCtrl[i].SlaveAddr == sCsncPara.sourceaddr)
                {
