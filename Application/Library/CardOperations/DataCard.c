...
//{
//	uint32_t	FlshRecCnt;	
//	uint32_t	FlshStartRecNum;
//	uint32_t	CardEndAddr;
//	uint32_t	CardStartAddr;
//	uint32_t	CardMaxRecNum;
//	uint32_t	NoReadRecNum;
//	uint16	CrcCheck;
...
//
//				CrcCheck = GetCrc16Check((uint8_t *)sCardFlg,sizeof(stcCardFlg) -2);
//
...
//					
//					CrcCheck = GetCrc16Check((uint8_t *)sCardFlg,sizeof(stcCardFlg) -2); //重写校验
//
//					sCardFlg->CrcCheck = CrcCheck;					
//					
...
//	uint16	CrcCheck;
	...
	ReadCard(CardAddrTmp,(uint8_t *)&WriteCardTimes,sizeof(WriteCardTimes));	
	...
	else
	{
		...
		ReadCard(CardAddrTmp,(uint8_t *)&sCardIndex,sizeof(stcCardIndex));
	...
	if((WriteCardTimes < 100)  && ( CardMaxRecNum >20))			//卡写满
	{
		...
		if( NoReadRecNum > 1 )												//未读记录条数须大于1才能读取数据	
		{
			if( StoreCardRec(FlshStartRecNum,FlshRecCnt,CardStartAddr,WriteCardTimes))		//返回1 写卡完成
			{
				...
				WriteCard(CardAddrTmp,(uint8_t *)&WriteCardTimes,sizeof(WriteCardTimes));	
				...
				if((l_sCardFlg.WriteCardTimes < 100)  && ( CardMaxRecNum > 20))			//卡写满
				{
					...
					if( NoReadRecNum > 1 )											//未读记录条数须大于1才能读取数据	
					{
						if( StoreCardRec(FlshStartRecNum,FlshRecCnt,CardStartAddr,l_sCardFlg.WriteCardTimes))		//返回1 写卡完成
						{
							...
							CrcCheck = GetCrc16Check((uint8_t *)&l_sCardFlg,sizeof(stcCardFlg) -2);
...
//出口参数:             无
//--------------------------------------------------------------------------------------------------
uint8_t	GetUnFihCardEvtFlg(void)
{
...
//出口参数:             无
//--------------------------------------------------------------------------------------------------
//uint8_t	GetWriteFlgErrCardEvtFlg(void)
//{
...
//----------------------------------------------------------------------
//函数名称:  	void    ReadCopyCardDisplay(uint8_t	 Sct)
//函数功能:  	将油尺记录FLASH转化为IC卡记录
//入口参数:   	FLASH记录
//出口参数:   	IC卡记录
//说明:                 
//-----------------------------------------------------------------------
void    ReadCopyCardDisplay(uint8_t	 Sct)
{
	uint8_t	i = 0;
...
//-------------------------------------------------------------------------------------------------
//函数名称:             uint8_t	DataCopyCard(stcCardFlg	* sCardFlg)
//函数功能:             置数据卡
//入口参数:             整个卡结构体
//出口参数:             是否成功
//--------------------------------------------------------------------------------------------------
//uint8_t	DataCopyCard(void)
//{
////	uint32_t	FlshRecCnt;	
////	uint32_t	FlshStartRecNum;
////	uint32_t	CardStartAddr;
...
////			(uint8_t *)&WriteCardTimes,sizeof(stcCardType));	
////	ReadCard(CARD_FLG_ADDR+sizeof(stcCardID)+sizeof(stcCardType)+sizeof(stcCardPara),
////			(uint8_t *)&sCardIndex[0],sizeof(stcCardIndex)*100);			
////	
